...
import { searchCalendarEvents } from "@/features/calendar/helpers/search-calendar-events";
...
function SearchResultItem({
  ...
  return (
    ...
    >
      <span className={cn("mt-1 size-2 shrink-0 rounded-full", color.dot)} aria-hidden />
      <div className="min-w-0 flex-1">
        <p className="text-xs text-muted-foreground tabular-nums">{formatEventDate(event)}</p>
        <p className="mt-0.5 text-sm leading-snug font-medium">{event.title}</p>
        {secondary ? (
...
export function CalendarEventSearch({
  ...
  const emptyMessage = !query.trim() ? "Etkinlik adı veya açıklama yazın." : "Sonuç bulunamadı.";
  ...
  return (
    <>
          ...
          >
            <Command shouldFilter={false}>
              <div className="hidden">
                <CommandInput value={query} onValueChange={setQuery} tabIndex={-1} />
              </div>
      ...
      <CommandDialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <CommandInput placeholder="Etkinlik ara..." value={query} onValueChange={setQuery} />
        <CommandList className="max-h-[min(24rem,calc(100vh-10rem))] overflow-y-auto overscroll-contain">
          ...
          {dialogResults.map((event) => (
            <SearchResultItem key={event.id} event={event} onSelect={handleSelect} />
          ))}
        </CommandList>
      </CommandDialog>
    </>
