...
describe('serializeCalendarFeed', () => {
  it('emits a valid all-day event block', () => {
    const events: CalendarEvent[] = [
      {
        ...
        allDay: true,
        categories: [{ slug: 'dini-gunler', name: 'Dini Günler', desc: null }],
    ...
    expect(ics).toContain('DESCRIPTION:Açıklama satırı')
    expect(ics).toContain('URL:https://example.com/kaynak')
    expect(ics).not.toContain('Kategori:')
    expect(ics).not.toContain('Kaynak:')
