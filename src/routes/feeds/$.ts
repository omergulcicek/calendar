...
import { fetchCategoryRows, resolveFeedSlugs } from "@/lib/feeds/load-feed-events.server";
    ...
    handlers: {
      GET: async ({ params, request }) => {
        ...
        if (feedName === "subscribe.ics") {
          ...
          const isAllSelected =
            slugs.length === knownSlugs.length && knownSlugs.every((slug) => slugs.includes(slug));
