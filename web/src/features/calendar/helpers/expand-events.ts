...
import type {
  ...
  Recurrence,
  SubjectRef,
} from "@/features/calendar/api/events";
...
function extractGregorianDates(dates: EventRow["gregorian_dates"]): EventGregorianDateRow[] {
  if (!dates) return [];
  return Array.isArray(dates) ? dates : [dates];
}

export function wallClockParts(date: Date) {
  const shifted = new Date(date.getTime() + EVENT_SOURCE_UTC_OFFSET_MINUTES * 60_000);
  return {
...
function isMultiDayAllDayRange(row: EventRow, rangeStart: Date, rangeEnd: Date): boolean {
...
function resolveRangeEnd(
  ...
  if (
    ...
    if (daySpan > 0) {
      ...
      return new Date(
        Date.UTC(endWall.getFullYear(), endWall.getMonth(), endWall.getDate(), 20, 59, 59),
      );
    }
  }
...
function buildCalendarEvent(
  ...
  const occStart = new Date(wallClockParts(chipStart).year, month, day, hours, minutes, seconds);
...
function makeGregorianYearlyOccurrence(
  ...
  return {
    ...
    displayEnd: wallClockInYear(displayRangeEnd, targetYear + displayYearSpan),
    allDay: row.is_all_day ?? false,
    categories: categories.map((c) => ({
      slug: c.slug,
      name: c.name,
...
function expandGregorianEvent(row: EventRow): CalendarEvent[] {
  ...
  const chipStart = row.occurrence_date ? new Date(row.occurrence_date) : rangeStart;
  ...
  const recurrence: Recurrence = row.recurrence === "none" ? "none" : "yearly";
...
function expandHijriEvent(row: EventRow): CalendarEvent[] {
  ...
  const overrideByYear = new Map(
    extractGregorianDates(row.gregorian_dates).map((entry) => [entry.gregorian_year, entry]),
  );
  ...
  for (let year = minYear; year <= maxYear; year++) {
    ...
    if (override) {
      ...
      chipStart = override.occurrence_date ? new Date(override.occurrence_date) : rangeStart;
      hasOccurrence = override.occurrence_date != null;
    } else if (row.hijri_month != null && row.hijri_day != null && row.hijri_rule == null) {
      const parts = findHijriOccurrenceInGregorianYear(year, row.hijri_month, row.hijri_day);
      if (!parts) continue;
    ...
    const durationMs = computeChipDurationMs(chipStart, rangeStart, rangeEnd, hasOccurrence, row);
